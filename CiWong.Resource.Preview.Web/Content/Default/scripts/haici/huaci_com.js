...
var ICIBA_HUAYI_COOKIE = "ICIBA_HUAYI_COOKIE";
var ICIBA_WORD = "";
var ICIBA_HUAYI_FLAG = 1; //是否是本页面第一次划
...
function ICIBA_HUAYI_searchword() {
    ...
    var i_s = iciba_huaci_url + 'dict.php?word=' + encodeURIComponent(word);
    ICIBA_HUAYI_mm(i_s);
}
        ...
        if (obj.style.display == "none" || !(mousePos.x > obj_left_x && mousePos.x < obj_right_x && mousePos.y > obj_left_y && mousePos.y < obj_right_y)) {
            ...
            if (txt && txt.length < 1000) {
                ...
                yiObj.onclick = function () {
                    ...
                    dict.innerHTML = '<div id="ICIBA_TOO_LONG" style="height:150px;padding-top:10px;padding-left:10px;" class="footer">您划取的内容太长，建议您去<a href="http://fanyi.baidu.com" target="_blank">百度翻译</a>页面。</div>';
                    obj.style.display = "block";
                    yiObj.style.display = "none";
                }
            }
...
function ICIBA_HUAYI_kg() {
    ...
    // alert(ICIBA_HUAYI_ALLOW);
    if (ICIBA_HUAYI_ALLOW == "1") {
        ICIBA_HUAYI_ALLOW = 1;
        iKg.innerHTML = "<a href='javascript:;' title='点击关闭即划即译'></a>";
        var kg = iKg.getElementsByTagName("a")[0];
        kg.className = "";
    } else {
        ICIBA_HUAYI_ALLOW = 0;
        iKg.innerHTML = "<a href='javascript:;' title='点击开启即划即译'></a>";
        var kg = iKg.getElementsByTagName("a")[0];
        kg.className = "icIBahyI-a2";
    }
    kg.onclick = function () {
        if (ICIBA_HUAYI_ALLOW) {
