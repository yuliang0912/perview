...
define(function (r, exports) {
    ...
    /*公共方法提炼*/
    var methods = {
        defaultTrainModes: function () {
            return [
                { id: 1, bname: "先讲后练", tip: "Teaching before learning", vt: 0 },
                { id: 2, bname: "先练后讲", tip: "Learning before teaching", vt: 1 },
                { id: 3, bname: "边讲边练", tip: "Teaching with learning", vt: 0 }
            ];
        },
        defaultWorkMode: function () {
            return [
                { id: 1, name: "自主练习" },
                { id: 2, name: "做作业" },
                { id: 3, name: "结果" }
        ...
        clearCurr: function (id) {
            $("div.trainBoxBar a,ul.lessonBar li").removeClass("curr");
            $("#" + id).addClass("curr");
        },
    ...
    var viewModel = {
        preView: function (settingOptions) {
            ...
            model.TrainModelList = ko.observableArray(methods.defaultTrainModes());
            model.trainmodelChange = function (data, event) {
                var currid = (event.currentTarget || event.srcElement).id;//当前点击的li                
                $("div.trainNavBar li").removeClass("curr");
                $("#" + currid).addClass("curr");
                model.trainmodel(data.id);
            ...
            model.papers.subscribe(function (data) {
                ...
                {
                    if (data.parts[1].list.length == 0) {
                        model.leftcss("padding:0px;");
                    }
                    else model.leftcss("");
            ...
            settings.workmodel == 1 && settings.id && ciwong.ajax.getJSON("/resource/get", { versionId: settings.id, moduleId: "2bea5300-972e-494f-b730-6cbc05f0a717" }, model.papers).done(function () {
                ciwong.colseLoading();
            });//异步获取自主练习的数据           
